...
echo "<script type='text/javascript'> document.location ='admin/dashboard.php'; </script>";
  ...
  $sql ="SELECT EmailId,Password,StudentId,Status FROM tblstudents WHERE EmailId=:username and Password=:password";
  $query= $dbh -> prepare($sql);
  $query-> bindParam(':username', $username, PDO::PARAM_STR);
  $query-> bindParam(':password', $password, PDO::PARAM_STR);
  $query-> execute();
...
<body class="font-primary">
    ...
    <div class="min-h-screen bg-neutral flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8 bg-white p-8 rounded-lg shadow-lg">
            <h4 class="text-2xl font-bold text-neutral-dark text-center">LOGIN FORM</h4>
            <form role="form" method="post" class="space-y-6">
                 <div class="space-y-1">
                     <label class="block text-sm font-medium text-neutral-dark">Enter Username or Email</label>
